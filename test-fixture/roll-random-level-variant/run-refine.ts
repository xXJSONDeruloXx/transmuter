/**
 * Test fixture: refine RollRandomLevelVariant to remove asm pins and barriers.
 *
 * RollRandomLevelVariant uses three register asm pins (`asm("r4")`, `asm("r6")`,
 * `asm("r5")`) and one asm barrier (`asm("" : "=r"(levelState) : "0"(addr))`).
 * This fixture tests whether `transmuter refine --guideline no-asm-pin`
 * can find equivalent C that produces the same assembly without them,
 * and then whether cleanup can simplify the result.
 *
 * Usage: npx tsx test-fixture/roll-random-level-variant/run-refine.ts
 */
...
async function main() {
  ...
  console.log('RollRandomLevelVariant refine — removing asm pins and barriers');
  ...
  const refiner = new Refiner({
    source: baseSource,
    functionName: 'RollRandomLevelVariant',
    ...
    sourcePrefix,
    concurrency: 4,
    maxIterationsPerViolation: 100_000,
    timeoutMsPerViolation: 120_000,
    seed: 42,
